...
BOOST_AUTO_TEST_CASE(apply_wrap_2)
{
	...
	typedef apply_wrap<first1, char>::type t11;
	typedef apply_wrap<last1 , char>::type t21;

	BOOST_CHECK((std::is_same<t11, char>::value));
	BOOST_CHECK((std::is_same<t21, char>::value));

	typedef apply_wrap<first2, char, int>::type t12;
	typedef apply_wrap<last2, int, char>::type t22;

	BOOST_CHECK((std::is_same<t12, char>::value));
	BOOST_CHECK((std::is_same<t22, char>::value));
	
	typedef apply_wrap<first3, char, int, int>::type t13;
	typedef apply_wrap<last3, int, int, char>::type t23;

	BOOST_CHECK((std::is_same<t13, char>::value));
	BOOST_CHECK((std::is_same<t23, char>::value));

	typedef apply_wrap<first4, char, int, int, int>::type t14;
	typedef apply_wrap<last4, int, int, int, char>::type t24;
